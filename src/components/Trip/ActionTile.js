...
export default function ActionTile({style, trip}) {
  ...
  const title = isActive
    ? `${i18n.t("How's")} ${place} ${i18n.t('treating you?')}`
    : isUpcoming
    ? `${i18n.t('Ready for')} ${place}?`
  ...
  const getSubtitle = () => {
    ...
    if (isUpcoming) {
      if (!noTasks) {
        return `${i18n.t('You still have')} ${trip.openTasks?.length} ${i18n.t(
          'tasks open ✅',
        )}`;
      }
  ...
  const getBottomInfo = () => {
    ...
    if (isActive || isUpcoming) {
      ...
      timeString = !isActive
        ? `${i18n.t('in')} ${difference} ${
            difference === 1 ? i18n.t('day') : i18n.t('days')
          }`
        : `${difference} ${i18n.t('days left')}`;
    }
    ...
    return (
      <View style={{marginTop: isActive ? 14 : 1}}>
        <View
          ...
          <View style={{flexDirection: 'row', marginLeft: 6}}>
            ...
            {isActive && (
              <View
                ...
                <Icon name="photograph" color={COLORS.neutral[300]} size={14} />
                <Body
                  type={1}
          ...
          <Subtitle type={1} text={timeString} color={typeColor} />
        </View>
        <View>
          <View
            style={{
              height: 7,
              backgroundColor: Utils.addAlpha(typeColor, 0.15),
              borderRadius: 10,
              marginTop: 5,
            }}
          />
  ...
  return (
    <AnimatedPressable
      ...
      style={[
        styles.container,
        style,
        {height: animatedHeight, borderRadius: animatedBorderRadius},
      ]}>
      <View>
        <View
          style={{
          ...
          <View style={[styles.typeContainer, {backgroundColor: typeColor}]}>
            ...
            />
            <EntIcon name="chevron-right" size={20} color={COLORS.shades[0]} />
          </View>
        </View>
