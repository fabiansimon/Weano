...
export default function ChecklistContainer({onPress, onLayout, sender}) {
  // STORES
  const {mutualTasks, privateTasks} = activeTripStore(
    state => state.activeTrip,
  );

  // STATE & MISC
  const [isPrivate, setIsPrivate] = useState(
    privateTasks?.length > mutualTasks?.length,
  );
    ...
    <Pressable style={styles.container} onPress={onPress}>
      ...
      {data?.length > 0 ? (
        ...
        <Body
          ...
          text={
            isPrivate
              ? i18n.t('No private tasks yet')
              : i18n.t('No mutual tasks yet')
          }
