...
export default function PollTile({
  ...
  const getPercentage = () => {
    ...
    return countedVotes === 0
      ? 0
      : ((item.votes.length / countedVotes) * 100).toFixed(1);
  };
    ...
    <Pressable
      ...
      <View style={[styles.optionTileContainer, {height}]}>
        <View
          style={[
            isActive
              ? styles.activeContainerOverlay
              : styles.inactiveContainerOverlay,
        ...
        <MaskedView
          ...
          />
          <View
            style={{backgroundColor: COLORS.neutral[300], flex: 1, height: 45}}
          />
        </MaskedView>
      </View>
    </Pressable>
